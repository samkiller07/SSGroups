...
export default async function BrandHomePage({ params }: BrandHomePageProps) {
  ...
  return (
    <div className={`flex flex-col flex-1 ${theme.containerBg}`}>
      ...
      <section className="border-b border-slate-800/80 bg-slate-950/70 py-6 px-4 sm:px-6 lg:px-8">
      ...
      {specializedServices.length > 0 && (
        <section className="bg-slate-950/80 border-y border-slate-800/80 py-14 sm:py-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {theme.whyPoints.map((pt, idx) => (
            <div key={idx} className={`p-6 rounded-3xl border ${theme.surfaceBg} space-y-3`}>
              <div className="p-3 rounded-2xl bg-slate-950 border border-slate-800 w-fit shadow-md">
                {pt.icon}
              </div>
              <h3 className="font-extrabold text-base text-white">{pt.title}</h3>
      ...
      <section className="border-t border-slate-800/80 bg-slate-950/90 py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto rounded-3xl bg-slate-900/60 border border-slate-800 p-8 sm:p-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          <div className="lg:col-span-7 space-y-4">
            <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-emerald-950/80 text-emerald-400 text-xs font-bold border border-emerald-800/60">
          ...
          <div className="lg:col-span-5 flex flex-col sm:flex-row lg:flex-col gap-3">
            ...
            {brand.googleMapsUrl && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="py-2.5 px-4 rounded-xl bg-slate-900 hover:bg-slate-800 text-slate-300 font-medium text-xs flex items-center justify-center gap-2 border border-slate-800 transition-colors"
              >
