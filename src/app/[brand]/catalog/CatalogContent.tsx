...
import { CatalogItem, Category, BrandConfig } from '@/types';
import { 
  Search, 
  Layers, 
  Filter, 
...
export function CatalogContent({ brandId, brand, initialItems, initialCategories }: CatalogContentProps) {
  const searchParams = useSearchParams();

  const [categories, setCategories] = useState<Category[]>(initialCategories);
  const [allItems, setAllItems] = useState<CatalogItem[]>(initialItems);
  ...
  // Client-side fallback sync with persistent Supabase catalog
  useEffect(() => {
    if (!brandId) return;
    getPublicCatalogAction(brandId)
      ...
      .catch((err) => console.error('[CatalogContent] Error loading public catalog:', err));
  }, [brandId]);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <aside className={`lg:col-span-3 space-y-6 ${mobileFiltersOpen ? 'block' : 'hidden lg:block'}`}>
          <div className="p-5 rounded-2xl bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 space-y-5 sticky top-24 shadow-sm">
            ...
            {/* Search Input */}
            <div>
              <label className="text-xs font-bold text-slate-800 dark:text-slate-300 block mb-1.5">Search</label>
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl pl-9 pr-3 py-2 text-xs text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none ${theme.searchBorder}`}
                />
              </div>
            </div>
            ...
            <div>
              <label className="text-xs font-bold text-slate-800 dark:text-slate-300 block mb-1.5">Item Type</label>
              <div className="grid grid-cols-3 gap-1 p-1 bg-slate-100 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 text-[11px]">
                <button
